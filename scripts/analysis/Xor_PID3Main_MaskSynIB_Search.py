...
def train_main(cfg, train_loader: DataLoader, device: str, val_loader: Optional[DataLoader] = None ) -> Tuple["FusionModel", Dict[str, Any]]:
    ...
    for epoch in range(cfg.epochs):
        ...
        for b in train_loader:
            ...
            # lambdas, vJt = ntk_strengths_onehot_source_debug(model, f, y, source)
            # lambdas, vJt, stats = ntk_strengths_onehot_source_debug( model, f, y, source, print_debug=False, steps=opt_steps)
            # lps = loss_per_source(f, y, source, steps=opt_steps)
            # print(lambdas)
            # print(cosine(vJt["Red"], vJt["Syn"]))
            opt.step()
        ...
        # --- add this right after your val_metrics block ---
        if val_loader is not None:
            val_metrics = _eval_epoch_main(model, val_loader, device, cfg.lambda_uni)
            val_metrics["epoch"] = epoch
            history["val"].append(val_metrics)

            # per-source val diagnostics (epoch-level)
            val_src = eval_loss_per_source(model, val_loader, device)
            val_acc = eval_by_source(model, val_loader, device)
            val_metrics.update(val_src)  # keep everything in one dict
            val_metrics.update(val_acc)  # keep everything in one dict
            ...
            if cur > check_with:
                history["best_val_fusion_acc"] = val_metrics["acc_fusion"]
                history["best_val_fusion_syn_acc"] = val_metrics["by_source"]["syn"]["acc"] if "syn" in val_metrics["by_source"] else np.nan
...
def train_synib_mstar( cfg, train_loader: DataLoader, device: str, val_loader: Optional[DataLoader] = None,) -> Tuple["FusionModel", Dict[str, Any]]:
    ...
    for lsp in lsp_vals:
        for kl in kl_vals:
            ...
            print("KL sweep: KL=", kl, " LSP=", lsp, " score=", score_kl)
    ...
    # if best_model is None:
    #     raise RuntimeError("No model was trained in kl sweep.", hist_kl)
...
def _prob_key(pu1: float, pu2: float, pred: float, psyn: float, pnone: float = 0.0) -> str:
    return f"({pu1:.2f},{pu2:.2f},{pred:.2f},{psyn:.2f})"
...
def generate_prob_settings(step: float, prior_u2: float, targets_psyn: List[float], scramble:bool=True) -> List[Tuple[float, float, float, float]]:
    ...
    return out
def sweep_nonoverlap(cfg0: Config, seeds: List[int], lr_vals: List[float], wd_vals: List[float], dropout_vals: List[float],
...
if __name__ == "__main__":
    ...
    # sweep_nonoverlap(cfg, seeds=[0,1,2], lr_vals=lr_vals, wd_vals=wd_vals, dropout_vals=dropout_vals, step=0.05, targets_psyn=[0.05, 0.1, 0.15, 0.2], select_by="val_acc", save_path=os.path.join(cfg.out_dir, "maintuned_valacc_nlrwddr.jsonl"))
    ...
    # ])
    if cfg.train_method=="synib_mlearned":
        # cfg.lsp_vals = [0.01, 0.1, 1.0, 3.0, 5.0, 10.0, 30.0]
        cfg.lsp_vals = [0.1, 1.0, 3.0, 5.0]
    else: cfg.lsp_vals = [0.0]
    #
    # print(f"######################################Training method: {cfg.train_method}######################################")
    save_path = os.path.join(cfg.out_dir, f"{cfg.train_method}_valsynacc_nlrwddr.jsonl")
