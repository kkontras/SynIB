...
case "${1:-}" in
  -h|--help)
    cat <<'EOF'
Usage:
  ./run/multibench/download_mustard.sh [options]
...
Examples:
  ...
  ./run/multibench/download_mustard.sh

  # Use a pre-downloaded zip (saves bandwidth if you already have it):
  ./run/multibench/download_mustard.sh --video-zip /path/to/mmsd_raw_data.zip

  # Use an already-cloned local repo + pre-downloaded zip:
  ./run/multibench/download_mustard.sh --local-repo /path/to/MUStARD --video-zip /path/to/mmsd_raw_data.zip

  # Use a local copy with utterances_final/ already present, symlink videos:
  ./run/multibench/download_mustard.sh --local-repo /path/to/MUStARD --symlink --output-root /data/mustard
EOF
    exit 0
    ;;
esac
