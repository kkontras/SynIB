...
RAW_ROOT="${RAW_ROOT:-./data/FactorCL_Raw}"
...
usage() {
  cat <<EOF
Usage:
  ./run/multibench/download_all_raw.sh
...
Notes:
  - MOSI first tries the legacy CMU raw zip URL.
  - If that fails, the script falls back to audEERING via audb (audio + transcripts + labels + splits).
...
echo "[raw-download] step=mosi"
MOSI_RAW_URL="${MOSI_RAW_URL}" \
PYTHON_BIN="${PYTHON_BIN}" \
RAW_SOURCES_ROOT="${RAW_SOURCES_ROOT}" \
./run/multibench/download_mosi_raw_sources.sh
...
./run/multibench/download_mustard.sh "${MUSTARD_ARGS[@]}"
