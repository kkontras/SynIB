...
RUN_SCRIPT_DEFAULT="./run/tqa/train_tier1.sh"

usage() {
  cat <<'EOF'
Usage:
  ./run/tier1/submit_tqa.sh --cache
  ./run/tier1/submit_tqa.sh --jobs-file run/tier1/tqa_tier1_jobs.txt
  ./run/tier1/submit_tqa.sh --run-script ./run/tqa/train_tier1.sh -- joint --fold 0 --lr 0.0005 --wd 0.0001
EOF
}

submit_cache() {
  qsub ./run/tier1/jobfile_tqa_cache.pbs     -v GPU="${GPU}",PROJECT_ROOT="${PROJECT_ROOT}",ENV_PATH="${ENV_PATH}"
}
...
submit_job() {
  ...
  qsub ./run/tier1/jobfile_tqa_train.pbs     -v GPU="${GPU}",PROJECT_ROOT="${PROJECT_ROOT}",ENV_PATH="${ENV_PATH}",RUN_SCRIPT="${run_script}",JOB_ARGS="${job_args}"
}
...
module swap cluster/dodrio/gpu_rome_a100_80_rhel9
