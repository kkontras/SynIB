...
def squad_convert_examples_to_features(
    ...
    if not is_training:
        ...
        all_switch = torch.tensor(
            [f.switch for f in features], dtype=torch.long)
        dataset = TensorDataset(
            all_input_ids,
            all_attention_masks,
            all_token_type_ids,
...
class SquadProcessor(DataProcessor):
    ...
    def __init__(self, train_file=None, dev_file=None, insert_title=False):
    ...
    def get_train_examples(self, filename=None):
        ...
        with open(self.train_file if filename is None else filename, "r", encoding="utf-8") as reader:
    ...
    def get_dev_examples(self, filename=None):
        ...
        with open(self.dev_file if filename is None else filename, "r", encoding="utf-8") as reader:
            input_data = json.load(reader)["data"]
    ...
    def _create_examples(self, input_data, set_type, tqdm_enabled=True):
        ...
        for entry in tqdm(input_data, disable=not tqdm_enabled):
            ...
            for paragraph in entry["paragraphs"]:
                ...
                if self.insert_title is True:
                    if isinstance(title, list):
                        title = title[0]
                    # remove distractor signal
                    if "_distractor" in title:
                        title = title.split("_distractor")[0]
                    # normalize the title
                    title = title[0].capitalize() + title[1:]
                    if "_parentSection:" in title:
                        orig_title = title.split("_parentSection:")[0]
                        if "title:" in title:
                            orig_title = orig_title.split("title:")[1]
                        elif "Title:" in title:
                            orig_title = orig_title.split("Title:")[1]
                        else:
                        ...
                        orig_title = title
                    context_text = "<title> " + orig_title + " </title> " + context_text
                ...
                for qa in paragraph["qas"]:
                    ...
                    if not is_impossible:
                        if is_training:
                            answer = qa["answers"][0] if isinstance(
                                qa["answers"], list) else qa["answers"]
                            answer_text = answer["text"]
                            start_position_character = answer["answer_start"]
                            if self.insert_title is True:
                                start_position_character = answer["answer_start"] + len(
                        ...
                        if "answers" in qa and len(qa["answers"]) > 0:
                            for answer in answers:
                                if answer["text"] == "yes":
                                    switch = 2
                                    answers.append(
                                        {"text": answer_text, "answer_start": -1})
                                if answer["text"] == "no":
                                    switch = 3
                                    answers.append(
                                        {"text": answer_text, "answer_start": -1})
...
# FIX ME:
# The file path should be dynamically updated.
...
class SquadResult(object):
    ...
    def __init__(self, unique_id, start_logits, end_logits, switch_logits, start_top_index=None, end_top_index=None, cls_logits=None):
        self.start_logits = start_logits
        self.end_logits = end_logits
        self.switch_logits = switch_logits
        self.unique_id = unique_id
