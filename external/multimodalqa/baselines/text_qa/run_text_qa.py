...
from squad_metrics import compute_predictions_logits, compute_predictions_logits_inference
...
# FIXME: fix to incorporate multiple loss.
...
# FIXME: add switch logits predictions
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        processor = SquadProcessor(
            train_file=args.train_file, dev_file=args.predict_file, insert_title=args.insert_title)
        if evaluate:
            examples = processor.get_dev_examples(args.predict_file)
        else:
            examples = processor.get_train_examples(args.train_file)

        # fic squad convert examples to features code
...
def main():
    ...
    # FIXME: fix me to accept xlm-roberta
    if args.model_type == "bert":
        print("loading bert based models...")
        model = BertForQuestionAnsweringYesNo.from_pretrained(
        ...
        model = RobertaForQuestionAnsweringYesNo.from_pretrained(
            args.model_name_or_path,
            from_tf=bool(".ckpt" in args.model_name_or_path),
            config=config,
            cache_dir=args.cache_dir if args.cache_dir else None,
