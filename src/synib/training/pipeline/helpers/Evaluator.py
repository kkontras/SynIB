...
from synib.utils.metrics.corr_metrics import (
...
class General_Evaluator:
    ...
    def evaluate(self, **eval_kwargs) -> Dict[str, Any]:
        ...
        def compute_metrics_for_head(pred_key: str, preds: torch.Tensor):
            ...
            if is_binary:
                ...
                try:
                    probs_pos = F.softmax(preds.float(), dim=-1)[:, 1].detach().cpu().numpy()
                    head_metrics["auroc"] = float(roc_auc_score(targets.detach().cpu().numpy(), probs_pos))
                except Exception:
                    head_metrics["auroc"] = float("nan")
    ...
    def ceu(self, total_preds, targets_tens):
        ...
        if len(targets_tens) == len(video_targets) == len(audio_targets) and (targets_tens.numpy() == video_targets).all() and (video_targets == audio_targets).all():
            ...
            logging.warning(
                "Skipping CEU for %s split due to target mismatch: targets=%s video_targets=%s audio_targets=%s",
                self.set,
                targets_tens.shape,
                video_targets.shape,
                audio_targets.shape,
