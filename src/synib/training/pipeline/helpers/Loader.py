...
from synib.utils.schedulers.linear_warmup_scheduler import LinearWarmupConstant
import wandb
...
class Loader():
    ...
    def load_pretrained_models(self):
        if "pretrained_model" in self.agent.config.model:
            pre = self.agent.config.model.pretrained_model
            if pre.get("use", False) and not self.agent.config.model.load_ongoing:
                strict = bool(pre.get("strict", True))
                if self.agent.accelerator.is_main_process:
                    self.agent.logger.info(
                        "Loading pretrained model from file {} (strict={})".format(
                            pre["dir"], strict
                        )
                    )
                checkpoint_path = self._resolve_checkpoint_path(pre["dir"])
                checkpoint = self._safe_torch_load(checkpoint_path, map_location="cpu")
                state_key = pre.get("state_key", "model_state_dict")
                state = checkpoint[state_key] if state_key in checkpoint else checkpoint
                result = self.agent.model.load_state_dict(state, strict=strict)
                if self.agent.accelerator.is_main_process:
                    missing = getattr(result, "missing_keys", []) or []
                    unexpected = getattr(result, "unexpected_keys", []) or []
                    self.agent.logger.info(
                        "Pretrained load: {} missing, {} unexpected keys".format(
                            len(missing), len(unexpected)
                        )
                    )
                    for k in missing[:10]:
                        self.agent.logger.info("  missing: {}".format(k))
                    if len(missing) > 10:
                        self.agent.logger.info("  ... {} more missing".format(len(missing) - 10))
                    for k in unexpected[:10]:
                        self.agent.logger.info("  unexpected: {}".format(k))
                    if len(unexpected) > 10:
                        self.agent.logger.info("  ... {} more unexpected".format(len(unexpected) - 10))
    ...
    def get_scheduler(self):
        if self.agent.config.scheduler.type == "cyclic":
            ...
            self.agent.scheduler = WarmupScheduler(optimizer=self.agent.optimizer,
                                                   ...
                                                   n_warmup_steps=self.agent.config.scheduler.warm_up_steps,
                                                   after_scheduler=after_scheduler)
        elif self.agent.config.scheduler.type == "linear_warmup_constant":
            self.agent.scheduler = LinearWarmupConstant(
