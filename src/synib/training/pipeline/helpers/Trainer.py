...
from synib.utils.data.flattendict import flatten_loss_dict
from synib.utils.data.to_device import to_device, to_float
import os
...
class Trainer():
    ...
    def train_steps(self):
        ...
        # PyTorch profiler: capture first N steps then exit. Gated by the
        # SYNIB_PROFILE_TORCH_STEPS env var so it doesn't affect normal runs.
        self._torch_profiler = None
        self._torch_profile_steps = int(os.environ.get("SYNIB_PROFILE_TORCH_STEPS", "0"))
        if self._torch_profile_steps > 0 and self.agent.accelerator.is_main_process:
            from torch.profiler import profile, ProfilerActivity, schedule, tensorboard_trace_handler
            outdir = os.environ.get("SYNIB_PROFILE_TORCH_DIR", "/tmp/torch_prof")
            os.makedirs(outdir, exist_ok=True)
            self._torch_profiler = profile(
                activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA],
                schedule=schedule(wait=1, warmup=1, active=self._torch_profile_steps, repeat=1),
                on_trace_ready=tensorboard_trace_handler(outdir),
                record_shapes=False,
                with_stack=False,
            )
            self._torch_profiler.__enter__()
            print(f"[PROF torch] capturing {self._torch_profile_steps} active steps -> {outdir}", flush=True)
        ...
        for current_epoch in range(self.agent.logs["current_epoch"], self.agent.config.early_stopping.max_epoch):
            ...
            for batch_idx, served_dict in pbar:
                ...
                if self._torch_profiler is not None:
                    ...
                    if batch_idx >= self._torch_profile_steps + 2:
                        self._torch_profiler.__exit__(None, None, None)
                        print(f"[PROF torch] trace saved, exiting", flush=True)
                        return
    ...
    def train_one_step(self, served_dict, **kwargs):
            _prof = os.environ.get("SYNIB_PROFILE_TIMER", "0") == "1"
            _print_every = int(os.environ.get("SYNIB_PROFILE_EVERY", "5"))
            if _prof:
            ...
            if _prof:
                torch.cuda.synchronize()
                _t_prep = time.perf_counter() - _t; _t = time.perf_counter()
            ...
            if _prof:
                torch.cuda.synchronize()
                _t_fwd = time.perf_counter() - _t; _t = time.perf_counter()
            ...
            if _prof:
                torch.cuda.synchronize()
                _t_bwd = time.perf_counter() - _t; _t = time.perf_counter()
            ...
            if _prof and (self.agent.logs.get("current_step", 0) % _print_every) == 0:
                print(f"[PROF step={self.agent.logs.get('current_step',0)}] prep={_t_prep*1000:.0f}ms fwd={_t_fwd*1000:.0f}ms bwd={_t_bwd*1000:.0f}ms total={(_t_prep+_t_fwd+_t_bwd)*1000:.0f}ms", flush=True)
