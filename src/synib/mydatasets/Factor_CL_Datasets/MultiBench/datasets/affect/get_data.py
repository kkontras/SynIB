...
try:
    ...
    _TORCHTEXT_IMPORT_ERROR = None
except Exception as exc:
    text = None
    _TORCHTEXT_IMPORT_ERROR = exc
from collections import defaultdict
from torch.nn.utils.rnn import pad_sequence
from torch.utils.data import DataLoader, Dataset

from synib.mydatasets.Factor_CL_Datasets.MultiBench.robustness.text_robust import add_text_noise
from synib.mydatasets.Factor_CL_Datasets.MultiBench.robustness.timeseries_robust import add_timeseries_noise
...
def _get_word_embeddings(word2id, save=False):
    if text is None:
        raise RuntimeError(
            "torchtext is required for GloVe embeddings in robust text evaluation, "
            "but it could not be imported."
        ) from _TORCHTEXT_IMPORT_ERROR
...
class Affectdataset(Dataset):
    ...
    def __getitem__(self, ind):
        ...
        # z-normalize data
        if self.z_norm:
            # Use unbiased=False so short sequences (len <= 1) do not trigger std warnings.
            # Clamp std to avoid divide-by-zero and keep outputs finite.
            eps = 1e-6
            vision_std = torch.std(vision, axis=0, keepdims=True, unbiased=False).clamp_min(eps)
            audio_std = torch.std(audio, axis=0, keepdims=True, unbiased=False).clamp_min(eps)
            text_std = torch.std(text, axis=0, keepdims=True, unbiased=False).clamp_min(eps)
            vision = torch.nan_to_num((vision - vision.mean(0, keepdims=True)) / vision_std)
            audio = torch.nan_to_num((audio - audio.mean(0, keepdims=True)) / audio_std)
            text = torch.nan_to_num((text - text.mean(0, keepdims=True)) / text_std)
        ...
        if self.task == "classification":
            label = torch.from_numpy(np.asarray(_get_class(tmp_label), dtype=np.int64))
        else:
            label = torch.from_numpy(np.asarray(tmp_label, dtype=np.float32))
...
def get_dataloader(
        ...
        raw_path='', z_norm=False) -> DataLoader:
    ...
    Args:
        ...
        raw_path (str, optional): Full path to the raw aligned source file used only for robust_test. Defaults to ''.
...
def _process_1(inputs: List):
    ...
    def _extract_label(label):
        ...
        if label.ndim > 1 and label.shape[1] > 1:
            return label.reshape(label.shape[1], label.shape[0])[0]
        return label
    ...
    for sample in inputs:
        ...
        labels.append(_extract_label(sample[-1]))

    inds_tensor = torch.as_tensor(inds).view(len(inputs), 1)
    labels_tensor = torch.stack([torch.as_tensor(lbl).view(-1) for lbl in labels], dim=0)
    if labels_tensor.shape[1] == 1:
        labels_tensor = labels_tensor.view(len(inputs), 1)
    return processed_input, processed_input_lengths, inds_tensor, labels_tensor
...
def _process_2(inputs: List):
    ...
    if labels_tensor.shape[1] == 1:
        labels_tensor = labels_tensor.view(len(inputs), 1)
    return processed_input[0], processed_input[1], processed_input[2], labels_tensor
