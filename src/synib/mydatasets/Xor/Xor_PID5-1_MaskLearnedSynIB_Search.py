...
def set_nonoverlap_signal_probs(cfg: Config, pu1: float, pu2: float, pred: float, psyn: float, pnone: float = 0.0) -> None:
    ...
    if abs(tot - 1.0) > 1e-5: raise ValueError(f"probs must sum to 1, got {pu1}+{pu2}+{pred}+{psyn}+{pnone}={tot}")
...
def train_synib_learned(cfg: Config, train_loader: DataLoader, val_loader: DataLoader, device: str, verbose: bool = True) -> Tuple[nn.Module, Dict[str, Any]]:
    ...
    for ep in range(int(cfg.epochs)):
        ...
        for b in train_loader:
            ...
            aggregate_masks["aggregate_masks_0"].append(masks["aggr_metrics"]["g0"])
            aggregate_masks["aggregate_masks_1"].append(masks["aggr_metrics"]["g1"])
        ...
        va_by_stats = eval_by_source(model, val_loader, device)
        va = {"epoch": ep, **va_stats, **va_by_stats}
        hist["train"].append(tr); hist["val"].append(va)

        # if va["acc_fusion"] > best_val:
        if va["syn"]["acc"] > best_val:
            # best_val = float(va["acc_fusion"])
            best_val = float(va["syn"]["acc"])
            print(f"We have a new best val {best_val} at epoch {ep}")
            best_state = {k: v.detach().cpu().clone() for k, v in model.state_dict().items()}
            hist["best_val_acc"] = float(va["acc_fusion"])
            hist["best_val_syn_acc"] = float(va["syn"]["acc"])
            hist["best_val_loss"] = float(va["loss_fusion"])
            hist["best_history"] = {"train": tr, "val": va}
            hist["best_epoch"] = ep
    ...
    new_aggregate_metrics = {}
    new_aggregate_metrics["aggregate_masks_0"]=defaultdict(list)
    for k in aggregate_masks["aggregate_masks_0"][0].keys():
        for am in aggregate_masks["aggregate_masks_0"]:
            new_aggregate_metrics["aggregate_masks_0"][k].append(am[k])

    new_aggregate_metrics["aggregate_masks_1"] = defaultdict(list)
    for k in aggregate_masks["aggregate_masks_1"][0].keys():
        for am in aggregate_masks["aggregate_masks_1"]:
            new_aggregate_metrics["aggregate_masks_1"][k].append(am[k])

    try:
        aggregate_masks['aggregate_masks_0'] = {k: torch.tensor(v).mean().item() for k, v in new_aggregate_metrics['aggregate_masks_0'].items() if len(v) > 0}
        aggregate_masks['aggregate_masks_1'] = {k: torch.tensor(v).mean().item() for k, v in new_aggregate_metrics['aggregate_masks_1'].items() if len(v) > 0}
        print(aggregate_masks['aggregate_masks_0'].keys())
        print(f"        [mask]x0/{cfg.learned_mask_method} obj={aggregate_masks['aggregate_masks_0']['obj']:.3f} obj_gt={aggregate_masks['aggregate_masks_0']['obj_gt']:.3f} overlap(syn)={aggregate_masks['aggregate_masks_0']['iou']:.3f} overlap(uni/res)={aggregate_masks['aggregate_masks_0']['iou_unires']:.3f} sparsity={aggregate_masks['aggregate_masks_0']['sp']:.3f} acc_g={aggregate_masks['aggregate_masks_0']['acc_u_t']:.3f} acc_gt={aggregate_masks['aggregate_masks_0']['acc_u_gt']:.3f} acc_clean={aggregate_masks['aggregate_masks_0']['acc_u_clean']:.3f}")
        print(f"        [mask]x1/{cfg.learned_mask_method} obj={aggregate_masks['aggregate_masks_1']['obj']:.3f} obj_gt={aggregate_masks['aggregate_masks_1']['obj_gt']:.3f} overlap(syn)={aggregate_masks['aggregate_masks_1']['iou']:.3f} overlap(uni/res)={aggregate_masks['aggregate_masks_1']['iou_unires']:.3f} sparsity={aggregate_masks['aggregate_masks_1']['sp']:.3f} acc_g={aggregate_masks['aggregate_masks_1']['acc_u_t']:.3f} acc_gt={aggregate_masks['aggregate_masks_1']['acc_u_gt']:.3f} acc_clean={aggregate_masks['aggregate_masks_1']['acc_u_clean']:.3f}")
    except:
        pass
    if best_state is not None:
        model.load_state_dict({k: v.to(device) for k, v in best_state.items()})

    print([f'{i["syn"]["acc"]:.2f}' for i in hist["val"]])
    return model, hist
    ...
    for seed in seeds:
        ...
        for lam_kl in kl_vals:
            for lam_sp in lam_sparsity_vals:
                ...
                val_best = hist["best_history"]["val"]
                ...
                row = {
                    ...
                    "val_best_syn_acc": float(val_best["syn"]["acc"]),
                ...
                if verbose:
                    ...
                    print(f"[seed={seed} l_ib={lam_kl:8.1e} l_sp={lam_sp:8.1e}] val_acc={row['val_best_acc']:.3f} val_syn_acc={row['val_best_syn_acc']:.3f} val_loss={row['val_best_loss']:.3f} | test_acc={row['test_acc']:.3f} syn={syn_acc:.3f} u1={u1_acc:.3f} red={red_acc:.3f}")
...
def main() -> None:
    ...
    save_path = os.path.join(cfg.out_dir, "synib_learneduni_sweep_deletethis.json")
    sweep_nonoverlap(cfg, seeds=[0], kl_vals=[1e0, 1e1, 1e2], lam_sparsity_vals=[0.1, 1],
