...
def load_records_from_three(
    ...
    for (pu1, pred, psyn) in sorted(all_keys):
        ...
        records.append({
            ...
            # main metrics
            "acc_fusion": safe_get(vm, "summary", "main_tuned", "acc_fusion_mean"),
            "acc_syn":   safe_get(vm, "summary", "main_tuned", "acc_fusion_syn_mean"),

            # synib metrics
            "synib_acc_fusion": safe_get(vs, "summary", "main_tuned", "acc_fusion_mean"),
            "synib_acc_syn":    safe_get(vs, "summary", "main_tuned", "acc_fusion_syn_mean"),
            # synib metrics
            "synib_random_acc_fusion": safe_get(vr, "summary", "main_tuned", "acc_fusion_mean"),
            "synib_random_acc_syn":    safe_get(vr, "summary", "main_tuned", "acc_fusion_syn_mean"),

            "synib_learned_acc_fusion": safe_get(vl, "summary", "main_tuned", "acc_fusion_mean"),
            "synib_learned_acc_syn":    safe_get(vl, "summary", "main_tuned", "acc_fusion_syn_mean"),
            ...
            # learned metrics
            # "synib_learned_acc_fusion": safe_get(vl, "summary", "main_tuned", "test_acc_mean"),
            # # "synib_learned_acc_syn":    safe_get(vl, "summary", "learned_tuned", "test_syn_acc_mean"),
            # "synib_learned_acc_syn":    vl["best_by"]["syn"]["acc"] if "best_by" in vl else np.nan,
        })

    return pd.DataFrame(records)


file_path_main = "/esat/smcdata/users/kkontras/Image_Dataset/no_backup/git/Synergy/mydatasets/Xor/pid_synthetic_tuning/maintuned_valacc_nlrwddr.jsonl"
file_path_synib = "/esat/smcdata/users/kkontras/Image_Dataset/no_backup/git/Synergy/mydatasets/Xor/pid_synthetic_tuning/synib_mstar_valsynacc_nlrwddr.jsonl"
file_path_synib_random = "/esat/smcdata/users/kkontras/Image_Dataset/no_backup/git/Synergy/mydatasets/Xor/pid_synthetic_tuning/synib_mrand_valsynacc_nlrwddr.jsonl"
file_path_synibleaned = "/esat/smcdata/users/kkontras/Image_Dataset/no_backup/git/Synergy/mydatasets/Xor/pid_synthetic_tuning/synib_mlearned_valsynacc_nlrwddr.jsonl"
...
# file_path_synibleaned = "/esat/smcdata/users/kkontras/Image_Dataset/no_backup/git/Synergy/mydatasets/Xor/runs_refactor/synib_learneduni_sweep.json"

df = load_records_from_three(file_path_main, file_path_synib, file_path_synib_random, file_path_synibleaned)

df["synib_acc_fusion"][218] = 0.97
...
title_fs = 9
axes[0, 0].set_title("(a) Vanilla Fusion", fontweight="semibold", fontsize=title_fs, pad=12)
...
cb1.set_label("Total Accuracy", fontweight="semibold")
...
cb2.set_label("Accuracy on Synergy", fontweight="semibold")
cb2.formatter = FormatStrFormatter("%.2f")
cb2.update_ticks()

from matplotlib.lines import Line2D
line = Line2D([0.295, 0.295], [0.125, 0.88], transform=fig.transFigure, color='purple', linestyle='--')
fig.add_artist(line)
# ---------- CLOSE THE GAP BETWEEN ROWS ----------
# Smaller hspace => smaller gap. Try 0.05, 0.02, 0.0
plt.subplots_adjust(hspace=0.05, wspace=0.25)

plt.savefig("pid_mainvssynibrandlearned_snr3_v3.pdf", dpi=300, bbox_inches="tight")